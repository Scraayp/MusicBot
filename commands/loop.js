...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (cmds)
      return interaction
        .reply({
          content: `You already have an active command here. ❌\nhttps://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${cmds}`,
          ephemeral: true,
        })
        .catch((e) => {});
          ...
          switch (button.customId) {
            case "loop":
              if (queue.repeatMode === 1)
                return interaction
                  .reply({
                    content: `You should disable loop mode of existing music first **(/loop)** ❌`,
                    ephemeral: true,
                  })
                  .catch((e) => {});
