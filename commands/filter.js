...
module.exports = {
  ...
  options: [
    {
      name: "filtre",
      description:
        "Type the filter you want to apply. (bassboost, 8D, nightcore, mono, karaoke)",
  ...
  run: async (client, interaction) => {
    ...
    if (!filtre)
      return interaction
        .reply({
          content: `Please enter a valid filter name. ❌\n\`bassboost, 8D, nightcore\``,
          ephemeral: true,
        })
        .catch((e) => {});
    ...
    if (!filter)
      return interaction
        .reply({
          content: `I couldn't find a filter with that name. ❌\n\`bassboost, 8D, nightcore\``,
          ephemeral: true,
        })
        .catch((e) => {});
    const filtersUpdated = {};
    filtersUpdated[filter] = queue["_activeFilters"].includes(filter)
      ? false
      : true;
    ...
    interaction
      .reply({
        content: `Applied: **${filter}**, Filter Status: **${
          queue["_activeFilters"].includes(filter) ? "Active" : "Inactive"
